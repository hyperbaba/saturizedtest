...
bash "install ruby-mysql" do
  user "root"
  group "root"
  code "apt-get -y install ruby-mysql"
end
...
bash "install zip" do
  user "root"
  group "root"
  code "apt-get -y install  zip unzip"
end
...
bash "change permisssion ov /var/www" do
  user "root"
  group "root"
...
if node['wp-install']['theme_unit_test'] == true then
 ...
 bash "Remove broken hotfix plugin from the tree" do
    user "vagrant"
    group "vagrant"
    cwd node['wp-install']['wpdir']
    code "mv /var/www/wp-content/plugins/hotfix /tmp/"
  end
